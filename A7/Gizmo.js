...
// changed name jsonUpdate -> arrUpdateRequests
// AI now a separate class
//fc 2.14.2016
// todo: move squad related state to Squad
// todo: set up init file that loads different terrain and tank configs

var timerFunction = null;
var gCycleInterval = 800;   // 800 ms
var gGameWidth     = 750;
var gGameHeight    = 590;
...
function initTanks() {
  ...
  var data = $.getJSON( "tanks.json", function( tankInitList ) {
    ...
    for (var i =0; i < tankInitList.length; i++) {
      ...
      if (tank.team === "alpha") {
          newTankInstance =   new TankAIAlpha(tank.team, tank.id,tank.x,tank.y,10,220,40);
          alphaTankList.push( newTankInstance );
          console.log(newTankInstance.sayHello() );
      }
      else {
          // set up Bravo Tanks - each start off with some x,y
          newTankInstance = new TankAIBravo(tank.team, tank.id,tank.x,tank.y,10,undefined,undefined);
